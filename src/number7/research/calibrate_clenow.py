...
def _memo(r: dict) -> str:
    ...
    header = (f"**Snapshot:** `{r['snapshot']}` · **Code:** `{r['code_sha']}` · "
              f"**Cash rate:** {r['cash_annual_rate']:.2%}")
    table_header = ("| profile | CAGR | Sharpe | max DD | avg gross | "
                    "return / unit exposure | beta | WFE | monkey pctile | DSR |")
