...
- **Position sizing — explicit four-stage pipeline** (review flagged the two sizing layers as ambiguous; order now normative, §14; stage order amended per risk-layer spec §11.1, signed off 2026-08-14): (1) *relative sizes* via Clenow ATR risk-parity, `shares = (equity × risk_factor) / ATR20`, **risk_factor = 10bp** (KB-08 sweet spot 8–15bp ⇒ ~20–25 positions); (2) *caps* — position, sector, liquidity; (3) *absolute level* via the portfolio vol-target scalar (the 10% target governs; ATR parity only distributes it) — caps precede the scalar because a cap-satisfying structural book stays cap-satisfying under any k ≤ 1, while scalar-first lets the ratchet re-risk into an uncapped structure; (4) *normalization* — if summed targets exceed 100% gross, scale all weights down proportionally (deterministic; the order layer never receives an infeasible book). **Per-position floor $1k on structural pre-scalar weights** (below ⇒ skip signal — frictions eat it; amendment 2026-08-14: funded positions may sit under $1k by the factor k — a minimum *order* size is an execution concern); ATR fault guard: if a name's computed size jumps >2× week-over-week from an ATR spike/gap, hold last size and log a sizing fault. Note: the KB has *no consensus* per-trade risk cap (0.5–1% vs 2% vs ≤5% in the same file); this pipeline is my pick — documented so it's a decision, not a drift.
...
- **Liquidity & concentration overlay (§14):** position ≤ min(10% equity, 25% of 20-day ADV, notional cap); no new entries in a name whose Corwin-Schultz spread estimate is >2× its rolling median; top-3 holdings combined ≤ 25%; portfolio beta to SPY within a declared band — measured on the *structural* (pre-scalar) book (amendment 2026-08-14: funded beta ≈ k × structural beta, so under a binding scalar the funded value tracks k, not selection drift); rolling average pairwise correlation of holdings monitored — sustained spike ⇒ vol-target scalar tightens (the correlation-breakdown brake).
...
- Production volatility estimator standardized once, used everywhere a *single* per-name or per-book σ forecast is needed: `σ = 0.30·σ_long-run + 0.70·σ_EWMA(span≈32d)` (KB-10). **Exception, declared per risk-layer spec §11.2 (signed off 2026-08-14):** the portfolio risk layer prices `w'Σw` and therefore uses its own covariance estimator — EWMA(halflife 63d) with 0.30 constant-correlation shrinkage. The two "0.30"s are unrelated (blend weight here, shrinkage intensity there); never conflate them, and never swap one estimator into the other's role without a declared spec amendment.
